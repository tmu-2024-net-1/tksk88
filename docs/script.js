       ...
       //ヘッダータイトル
        ...
        function showImages(type) {
            ...
            newImage.innerHTML = `
                <img src="images/${type}.png" width="80px" height="80px" alt="${type}">
            `;
    ...
    const types = ["musi", "tori", "sakana"];
    types.forEach(type => {
        var img = document.querySelector(`#${type}-container .image-item img`);
        if (img) {
            var isGif = img.src.endsWith(`${type}.gif`);
            var newSrc = isGif ? `images/${type}.png` : `images/${type}.gif`;
            img.src = newSrc;
        }
    });
